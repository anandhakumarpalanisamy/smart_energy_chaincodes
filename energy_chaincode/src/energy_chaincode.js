...
async buy_energy(stub, args, thisClass) {
    //   0       1
    // 'name', 'bob'
    if (args.length < 3) {
    ...
    if (!fromuserAsBytes || !fromuserAsBytes.toString()) {
      throw new Error('fromuserAsBytes does not exist');
    }

    let touserAsBytes = await stub.getState(to_user);
    if (!touserAsBytes || !touserAsBytes.toString()) {
      throw new Error('touserAsBytes does not exist');
    }


    let marbleFromTransfer = {};
    try {
        marbleFromTransfer = JSON.parse(fromuserAsBytes.toString()); //unmarshal
    } catch (err) {
      let jsonResp = {};
      jsonResp.error = 'Failed to decode JSON of: ' + from_user;
      throw new Error(jsonResp);
    }
    console.info(marbleFromTransfer);
    if(marbleFromTransfer.amount < amount) {
        let jsonResp = {};
        jsonResp.error = 'No sufficient balance for ' + from_user;
        throw new Error(jsonResp);
    }
    else {
    ...
    let marbleToTransfer = {};
    try {
      marbleToTransfer = JSON.parse(touserAsBytes.toString()); //unmarshal
    } catch (err) {
      let jsonResp = {};
      jsonResp.error = 'Failed to decode JSON of: ' + from_user;
      throw new Error(jsonResp);
